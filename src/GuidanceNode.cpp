...
bool            show_images = 0;
uint8_t         verbosity = 0;
e_vbus_index	DOWN_CAMERA_ID = e_vbus5;
e_vbus_index	BACK_CAMERA_ID = e_vbus3;
DJI_lock        g_lock;
DJI_event       g_event;
...
int my_callback(int data_type, int data_len, char *content)
{
  ...
  if (e_image == data_type && NULL != content)
	...
	//publish depth image
	cv_bridge::CvImage depth_8;
	g_depth.copyTo(depth_8.image);
	depth_8.header.frame_id  = "guidance";
	depth_8.header.stamp	  = ros::Time::now();
	depth_8.encoding	  = sensor_msgs::image_encodings::MONO8;
	depth_image_pub.publish(depth_8.toImageMsg());
      }
#endif
      if ( data->m_depth_image[BACK_CAMERA_ID] ){
	memcpy(g_depth.data, data->m_depth_image[BACK_CAMERA_ID], IMAGE_SIZE * 2);
	g_depth.convertTo(depth8, CV_8UC1);
	if (show_images) {
	  imshow("back_depth", depth8);
	}
	//publish depth image
	cv_bridge::CvImage depth_8;
	g_depth.copyTo(depth_8.image);
	...
	depth_8.header.stamp	  = ros::Time::now();
	depth_8.encoding	  = sensor_msgs::image_encodings::MONO8;
	back_depth_image_pub.publish(depth_8.toImageMsg());
      }
      if ( data->m_depth_image[DOWN_CAMERA_ID] ){
	memcpy(g_depth.data, data->m_depth_image[DOWN_CAMERA_ID], IMAGE_SIZE * 2);
	g_depth.convertTo(depth8, CV_8UC1);
	if (show_images) {
	  imshow("down_depth", depth8);
	}
	//publish depth image
	cv_bridge::CvImage depth_8;
	g_depth.copyTo(depth_8.image);
	...
	depth_8.header.stamp	  = ros::Time::now();
	depth_8.encoding	  = sensor_msgs::image_encodings::MONO8;
	down_depth_image_pub.publish(depth_8.toImageMsg());
      }
      if (show_images)  key = waitKey(1);
    }
...
int main(int argc, char** argv)
{
  ...
  back_depth_image_pub			= my_node.advertise<sensor_msgs::Image>("/guidance/back/depth_image",1);
  down_depth_image_pub			= my_node.advertise<sensor_msgs::Image>("/guidance/down/depth_image",1);
  ...
  RETURN_IF_ERR(err_code);
#endif 
  err_code = select_depth_image(BACK_CAMERA_ID);
  RETURN_IF_ERR(err_code);
  err_code = select_depth_image(DOWN_CAMERA_ID);
  RETURN_IF_ERR(err_code);
  ...
  //para.m_camera_pair_index = BACK_CAMERA_ID;
  ...
  while (ros::ok())
	...
	if(key=='j' || key=='k' || key=='m' || key=='n'){
	  ...
	  std::cout<<"Setting exposure parameters....SensorId="<<BACK_CAMERA_ID<<std::endl;
	  para.m_camera_pair_index = BACK_CAMERA_ID;
	  set_exposure_param(&para);
	  std::cout<<"Setting exposure parameters....SensorId="<<DOWN_CAMERA_ID<<std::endl;
	  para.m_camera_pair_index = DOWN_CAMERA_ID;
	  set_exposure_param(&para);

	  key = 0;
	}
	else if (key == 'q'){// switch image direction
